...
class MarketTests(unittest.TestCase):
    ...
    def test_createStore(self):
        self.__storeId1 = self.__market.createStore("foot-locker", self.__member1.getUserID(), self.__bank1,
                                                    self.__address2)  # storeId1 = 0 , founder: member1
        self.assertTrue(1, self.__storeId1)

        self.__storeId2 = self.__market.createStore("Decathlon", self.__member2.getUserID(), self.__bank1,
                                                    self.__address3)  # storeId2 = 1, founder: member2
    ...
    def test_addProductToStore(self):
        ...
        self.assertTrue(self.__market.addProductToStore(1, self.__member1.getUserID(), self.__product1))
        self.assertTrue(self.__market.addProductToStore(1, self.__member1.getUserID(), self.__product2))
        self.assertTrue(self.__market.addProductToStore(2, self.__member2.getUserID(), self.__product3))

    def test_addProductToStore_FAIL(self):
        # try to add a product to a non-existing store
        self.assertRaises(Exception,
                          lambda: self.__market.addProductToStore(1, self.__member1.getUserID(), self.__product1))
        self.test_createStore()
        # member without the permission tries to add a product
        self.assertRaises(Exception,
                          lambda: self.__market.addProductToStore(1, self.__member2.getUserID(), self.__product1))
        # try to add a product that all ready exists
        self.assertTrue(self.__market.addProductToStore(1, self.__member1.getUserID(), self.__product1))
        self.assertRaises(Exception,
                          lambda: self.assertTrue(self.__market.addProductToStore(1, self.__member1.getUserID(),
                                                                                  self.__product1)))

    def test_addProductQuantity(self):
        self.test_addProductToStore()
        try:
            self.__market.addProductQuantityToStore(1, self.__member1.getUserID(), self.__product1.getProductId(), 100)
            self.__market.addProductQuantityToStore(1, self.__member1.getUserID(), self.__product2.getProductId(), 92)
            self.__market.addProductQuantityToStore(2, self.__member2.getUserID(), self.__product3.getProductId(), 31)
    ...
    def test_addProductQuantity_Fail(self):
        ...
        self.assertRaises(Exception, lambda: self.__market.addProductQuantityToStore(1, self.__member1.getUserID(),
    ...
    def test_addProductToCart(self):
        ...
        self.assertTrue(self.__market.addProductToCart(self.__member1.getUserID(), 1,
                                                       self.__product1.getProductId(), 7))
        self.assertTrue(self.__market.addProductToCart(self.__member1.getUserID(), 2,
                                                       self.__product3.getProductId(), 3))
        self.assertTrue(self.__market.addProductToCart(self.__member2.getUserID(), 1,
                                                       self.__product2.getProductId(), 10))
    ...
    def test_addProductToCart_Fail(self):
        ...
        self.assertRaises(Exception, lambda: self.__market.addProductToCart(self.__member1, 1,
    ...
    def test_removeProductFromCart(self):
        ...
        self.assertTrue(self.__market.removeProductFromCart(1, self.__member1.getUserID(),
                                                            self.__product1.getProductId()))
        print(self.__market.getCart(self.__member1.getUserID()))
    ...
    def test_purchaseCart(self):
        self.test_addProductToCart()
        self.assertTrue(self.__market.purchaseCart(self.__member1.getUserID(), self.__bank1))
    ...
    def test_removeStore(self):
        self.assertEqual(self.__market.removeStore(1, self.__member1.getUserID()), "Store removed successfully!")
