...
def initialize_system():
    if not Initialized.objects.all().exists():
        ...
        guest1: GuestDTO = user_service.enterSystem().getData()
        guest2: GuestDTO = user_service.enterSystem().getData()
        guest3: GuestDTO = user_service.enterSystem().getData()
        guest4: GuestDTO = user_service.enterSystem().getData()
        guest5: GuestDTO = user_service.enterSystem().getData()
        ...
        user1: MemberDTO = user_service.memberLogin(guest1.getUserID(), "ori", "1234").getData()
        ...
        role_service.addProductQuantityToStore(store1.getStoreId(), user1.getUserID(), computer.getProductId(), 420)
        camera: ProductDTO = role_service.addProductToStore(store1.getStoreId(), user1.getUserID(), "Camera", 1000,
                                                            "Electric Devices", 20, ["Camera"]).getData()
        ...
        cases: ProductDTO = role_service.addProductToStore(store1.getStoreId(), user1.getUserID(), "Phone Case", 50,
                                                           "Phone Accessories", 5, ["Phone Cases"]).getData()
        ...
        user: MemberDTO = user_service.memberLogin(guest2.getUserID(), "bar", "1234").getData()
        ...
        user: MemberDTO = user_service.memberLogin(guest3.getUserID(), "rotem", "1234").getData()
        ...
        kfir: MemberDTO = user_service.memberLogin(guest4.getUserID(), "kfir", "1234").getData()
        user_service.addProductToCart(kfir.getUserID(), 0, 0, 2)
        user_service.purchaseCart(kfir.getUserID(), "1111111111111110", 9, 2027, "kfir", 999, "222222221")
        member_service.logoutMember("kfir")
        niv: MemberDTO = user_service.memberLogin(guest5.getUserID(), "niv", "1234").getData()
        ...
        user_service.purchaseCart(niv.getUserID(), "1111111111111111", 9, 2027, "Niv", 999, "222222222")
        member_service.logoutMember("niv")
    Initialized.objects.get_or_create(is_initialized=True)
