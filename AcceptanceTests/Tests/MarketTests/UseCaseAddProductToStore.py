...
class UseCaseAddProduct(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.proxy_user.register("testUser1", "1234", "0540000000", 123, [], "Israel", "Beer Sheva", "Rager", 1, "testBank")
        self.__guestId2 = self.proxy_user.login_guest().getData().getUserID()
        self.proxy_user.register("testUser2", "1234", "0540000000", 123, [], "Israel", "Beer Sheva", "Rager", 1, "testBank")
        self.__guestId3 = self.proxy_user.login_guest().getData().getUserID()
        self.proxy_user.register("testUser3", "1234", "0540000000", 123, [], "Israel", "Beer Sheva", "Rager", 1, "testBank")
        ...
        # Create 3 stores
        self.store_id1 = self.proxy_user.open_store("testStore1", self.user_id1, 123, None, "Israel", "Beer Sheva", "Rager", 1, 00000).getData().getStoreId()
        self.store_id2 = self.proxy_user.open_store("testStore2", self.user_id2, 123, None, "Israel", "Beer Sheva", "Rager", 1, 00000).getData().getStoreId()
        self.store_id3 = self.proxy_user.open_store("testStore3", self.user_id3, 123, None, "Israel", "Beer Sheva", "Rager", 1, 00000).getData().getStoreId()

    def test_addProductPositive(self):
        p1_id = self.proxy_market.add_product_to_store(self.store_id1, self.user_id1, "testProduct1", 10, "testCategory", 15, ["test"]).getData().getProductId()
        p2_id = self.proxy_market.add_product_to_store(self.store_id1, self.user_id1, "testProduct2", 100, "testCategory", 10, ["test"]).getData().getProductId()
        storeProductsIds = [pId for pId in self.proxy_market.get_store_by_ID(self.store_id1).getData().getProducts().keys()]
