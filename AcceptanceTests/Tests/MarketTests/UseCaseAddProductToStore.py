...
class UseCaseAddProduct(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.proxy_user.register(cls.__guestId1, "testUser1", "1234", "0540000000", 123, [], "Israel", "Beer Sheva", "Rager", 1,
                                "testBank")
        cls.__guestId2 = cls.proxy_user.login_guest().getData().getUserID()
        cls.proxy_user.register(cls.__guestId2, "testUser2", "1234", "0540000000", 123, [], "Israel", "Beer Sheva", "Rager", 1,
                                "testBank")
        cls.__guestId3 = cls.proxy_user.login_guest().getData().getUserID()
        cls.proxy_user.register(cls.__guestId3, "testUser3", "1234", "0540000000", 123, [], "Israel", "Beer Sheva", "Rager", 1,
                                "testBank")

        cls.user_id1 = cls.proxy_user.login_member("testUser1", "1234").getData().getUserID()
        cls.user_id2 = cls.proxy_user.login_member("testUser2", "1234").getData().getUserID()
        cls.user_id3 = cls.proxy_user.login_member("testUser3", "1234").getData().getUserID()
    ...
    def test_addProductNegativePrice(self):
        # price is negative
        self.assertTrue(
            self.proxy_market.add_product_to_store(self.store_id1, self.user_id1, "testProduct", -20, "testCategory",
                                                   ["test"]).isError())
        self.assertTrue(
            self.proxy_market.add_product_to_store(self.store_id2, self.user_id2, "testProduct", -1, "testCategory",
                                                   ["test"]).isError())

    def test_addProductNoCategory(self):
        # no category
        self.assertTrue(Exception,
                        lambda: self.proxy_market.add_product_to_store(self.store_id1, self.user_id1, "testProduct1",
                                                                       10, None, ["test"]).isError())
        self.assertTrue(Exception,
                        lambda: self.proxy_market.add_product_to_store(self.store_id2, self.user_id2, "testProduct2",
                                                                       10, None, ["test"]).isError())
