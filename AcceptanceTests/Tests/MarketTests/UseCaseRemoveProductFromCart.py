...
class UseCaseRemoveProduct(unittest.TestCase):
    ...
    def test_removeProductPositive(self):
        ...
        self.assertTrue(self.proxy_user.add_product_to_cart(self.user_id, self.store_id, self.prod.getProductId(), 10).isError())

        self.prod1 = self.proxy_market.add_product_to_store(self.store_id, self.user_id, "testProduct", 10,
                                                           "testCategory", 10, ["testKeyWord"]).getData()
        self.assertTrue(self.proxy_market.remove_product_from_store(self.store_id, self.user_id, self.prod1.getProductId()).getData())
        self.assertTrue(self.proxy_user.add_product_to_cart(self.user_id, self.store_id, self.prod.getProductId(), 10).isError())
